...
from vtask.utils.hls.parser import parse_master_playlist, merge_intersected_strings, parse_media_playlist


def test_master_playlist():
    print()
    with open(path_join(find_project_root(), "dev", "hls", "test_master.m3u8"), "r") as f:
        m3u8 = f.read()
...
def test_media_playlist():
    print()
    with open(path_join(find_project_root(), "dev", "hls", "test_media.m3u8"), "r") as f:
        m3u8 = f.read()
