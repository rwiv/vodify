...
class SoopVideoDownloader:
    ...
    def download_one(self, title_no: int) -> str | list[str]:
        video_info = self.client.get_video_info(title_no)
        bj_id = video_info.bj_id
        path_infos: list[SegmentsPathInfo] = []
        for i, m3u8_info in enumerate(video_info.m3u8_infos):
            video_path = self.__download_segments(m3u8_info.video_master_url, bj_id, f"{i}_video", True)
            audio_path = self.__download_segments(m3u8_info.audio_media_url, bj_id, f"{i}_audio", False)
        ...
        if len(path_infos) == 1:
            return self.__remux_video(path_infos[0], bj_id, f"{title_no}.mp4")

        if self.ctx.concat:
            out_mp4_path = self.__concat_video_parts(title_no, video_info, path_infos)
            return out_mp4_path

        result = []
        for i, segments_path in enumerate(path_infos):
            file_name = f"{title_no}_{i}.mp4"
            out_mp4_path = self.__remux_video(segments_path, bj_id, file_name)
            result.append(out_mp4_path)
        return result

    def __download_segments(self, m3u8_url: str, bj_id: str, file_name: str, is_video: bool) -> str:
        if is_video:
            urls = self.hls.get_seg_urls_by_master(m3u8_url, None)
        else:
            urls = self.hls.get_seg_urls_by_media(m3u8_url, None)
        segments_path = path_join(self.tmp_dir_path, bj_id, file_name)
        if self.ctx.is_parallel:
            return asyncio.run(self.hls.download_parallel(urls=urls, segments_path=segments_path))
        else:
            return asyncio.run(self.hls.download(urls=urls, segments_path=segments_path))

    def __remux_video(self, path_info: SegmentsPathInfo, bj_id: str, file_name: str):
        tmp_mp4_path = mux_to_mp4(path_info)
    ...
    def __concat_video_parts(self, title_no: int, video_info: SoopVideoInfo, path_infos: list[SegmentsPathInfo]) -> str:
        bj_id = video_info.bj_id
        os.makedirs(path_join(self.out_dir_path, bj_id), exist_ok=True)
...
def mux_to_mp4(info: SegmentsPathInfo) -> str:
    # validate segments
    video_seg_names = os.listdir(info.video_segments_path)
    video_seg_names.sort(key=lambda x: int(stem(x)))
    audio_seg_names = os.listdir(info.video_segments_path)
    ...
    check_stream(path_join(info.video_segments_path, video_seg_names[0]), True)
    check_stream(path_join(info.audio_segments_path, audio_seg_names[0]), False)

    # merge ts files
    video_path = merge_ts(info.video_segments_path)
    shutil.rmtree(info.video_segments_path)
    audio_path = merge_ts(info.audio_segments_path)
    shutil.rmtree(info.audio_segments_path)

    merged_ts_path = path_join(info.out_dir_path, info.bj_id, f"{info.video_name}.ts")
    command = ["ffmpeg", "-i", video_path, "-i", audio_path, "-c", "copy", merged_ts_path]
    subprocess.run(command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def check_stream(file_path: str, is_video: bool):
    command = ["ffprobe", "-v", "quiet", "-print_format", "json", "-show_streams", file_path]
    result = subprocess.run(command, check=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if result.returncode != 0:
        raise Exception(f"ffprobe failed for {file_path}: {result.stderr}")
    if result.stderr:
        raise Exception(f"ffprobe returned stderr: {result.stderr}")

    info = FfprobeInfo(**json.loads(result.stdout))
